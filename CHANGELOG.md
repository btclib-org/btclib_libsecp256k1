...
- **`reasonless-coverage-pragma` refuses a `#`-comment `pragma: no cover`
  ...
  `btclib-org/.github`'s at `69a47e6`, with no `files:` narrowing them.
  The tree is green under the hook at the run that adds it.
...
- **`check-changelog` refuses a `###` heading repeated in this file's
  ...
  it lets the hook scripts beside it: what it prints is its report.
- **`tests/check_changelog_test.py` holds the script at the coverage
  floor** (issue btclib-org/.github#21): `[tool.coverage.run]` measures
  `.github/scripts` by path, so a script no test reaches reports at 0%
  and the union `test.yml` gates at 100% falls short of it. The tests
  are `btclib-org/.github`'s own for the script, in this tree's by-path
  shape, plus what a suite with no floor never needed -- a changelog
  with no release heading, and the entry-point guard run as `__main__`,
  the way the hook scripts beside it are tested.
- **`check-docstring-first` passes over that script** (issue
  btclib-org/.github#995): the script's `_BLANK_LINE` carries an
  attribute docstring the hook reads as a second module docstring, and
  `btclib-org/.github` runs no `check-docstring-first` of its own, so
  the file passes the gate there and fails it here. The exclusion is the
  price of carrying the file byte for byte rather than repairing a copy
  the verbatim test would refuse, and it goes the day the script passes
  the hook.
- **`toml-comment-width`'s `name:` states the pattern's own predicate,
  ...
  v0.8.0.5's *The `toml-comment-width` comment states its pattern's own
  predicate* left the `name:` to a decision every tree carrying the hook
  takes together, and `btclib-org/.github` took it at `51f717e`.
